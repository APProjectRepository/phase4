				...
				else
				{
					...
					while (true)
					{
						try
						{
							...
							// Check the username, password, and connection permission for each received command. 
							userInfo = ds.userInfo(username);	
							if (!username.equalsIgnoreCase(userInfo[0]))
							{
								pw.println("ERROR: Invalid username");
							...
							if (!(q instanceof PermissionQuery))
							{
								...
								// The user wants to manipulate other users' tables.
								{
								...
								// The user wants to manipulate their own tables.
								{	
						...
						{
							pw = new PrintWriter(client.getOutputStream());
							pw.println(new QueryResult(false, e.getMessage(), "", 0).message);
							pw.flush();
						}
					}
				}
