...
import query.DropTableQuery;
import query.InsertIntoQuery;
import query.PermissionQuery;
import query.Query;
import query.SelectTableDataQuery;
...
public class DataStorage {
	...
	public QueryResult execute(Query q) throws QueryException {
		...
		if (q instanceof SingleTableQuery) {
			...
			if (q instanceof InsertIntoQuery) {
				return insertIntoTable(table,
						((InsertIntoQuery) q).getRowData());
			}
			if (q instanceof DeleteFromTableQuery) {
				return deleteFromTable(table,
						((DeleteFromTableQuery) q).getCondition(),
						((DeleteFromTableQuery) q).cascade);
			}
			if (q instanceof UpdateTableDataQuery) {
				return updateTableData(table,
						((UpdateTableDataQuery) q).getCondition(),
						((UpdateTableDataQuery) q).getNewValues());
			}
			if (q instanceof SelectTableDataQuery) {
				return loadTableData(table,
						((SelectTableDataQuery) q).getCondition(),
						((SelectTableDataQuery) q).getColumns());
			}
			if (q instanceof PermissionQuery) {
				return setPermissionTable(table,
						((PermissionQuery) q).getArgs(),
						((PermissionQuery) q).getOwner());
			}
		}
	...
	// GRANT or REVOKE
	private QueryResult setPermissionTable(Table table, Vector<String> args,
			String owner) throws QueryException {
		...
		userOwnerTable = userOwnerTable.substring(1,
				userOwnerTable.length() - 1);
		...
		UpdateTableDataQuery q1 = new UpdateTableDataQuery("permissions", vec1,
				condStr);
		QueryResult res1 = this.execute(q1);

		// Insert a row into the 'permissions' if the appropriate row does not
		// exist.
		if (res1.message.contains("0")) {
			...
			for (int i = argsNum + 2; i < 2 * argsNum; ++i)
				vec2.addElement(args.elementAt(i));
			InsertIntoQuery q2 = new InsertIntoQuery("permissions", vec2,
					argsNum - 1);
			this.execute(q2);
		}
		...
		message = message.concat("ON "
				+ args.elementAt(argsNum + 1).substring(1,
						args.elementAt(argsNum + 1).length() - 1));
		if (args.elementAt(argsNum + 2).equals("true"))
			message = message.concat(" granted to ");
		else
			message = message.concat(" revoked from ");
		message = message.concat(args.elementAt(argsNum).substring(1,
				args.elementAt(argsNum).length() - 1));
	...
	// SELECT
	synchronized private QueryResult loadTableData(Table table,
			SimpleCondition condition, List<TableColumn> columns)
		...
		try {
			...
			for (int i = 0; i < rowNum; ++i) {
				...
				if (condition.value == null
						|| tempStr[index1].equals(condition.value)) {
	...
	// UPDATE
	synchronized private QueryResult updateTableData(Table table,
			SimpleCondition condition, List<RowColumnValue> newValues)
			throws QueryException {
		if (!tableExists(table.name))
			throw new QueryException(2, table.name);
			...
			for (int i = 0; i < rowNum; ++i) {
				...
				if (condition.value == null
					...
					if (table.name.equalsIgnoreCase("users")) {
						...
						} else if (newValues.get(0).getColName()
								.equals("password"))
							resID = 6;
					}
					for (int j = 0; j < index2.size(); ++j)
		...
		return new QueryResult(true, updatedNum + " row(s) updated",
				condition.value, resID);
	}

	// DELETE
	synchronized private QueryResult deleteFromTable(Table table,
			SimpleCondition condition, boolean cascade) throws QueryException {
		if (!tableExists(table.name))
			throw new QueryException(2, table.name);
		...
		return new QueryResult(true, deletedNum + " row(s) deleted",
				condition.value, 7);
	}

	// INSERT
	synchronized private QueryResult insertIntoTable(Table table,
			TableRow rowData) throws QueryException {
		if (!tableExists(table.name))
			throw new QueryException(2, table.name);
		...
		return new QueryResult(true, "One row inserted into " + table.name,
				rowData.getColValue(0), resID);
	}

	// DROP
	synchronized private QueryResult dropTable(Table table)
			throws QueryException {
		if (!tableExists(table.name))
			throw new QueryException(2, table.name);
	...
	// CREATE
	synchronized private QueryResult createTable(Table table)
			throws QueryException {
		if (tableExists(table.name))
	...
	public boolean userPermitted(String username, SingleTableQuery q1)
			throws QueryException {
		Vector<String> vec = new Vector<String>();
		...
		condStr[0] = "user.owner.table";
		condStr[1] = userOwnerTable;
		SelectTableDataQuery q2 = new SelectTableDataQuery("permissions", vec,
				condStr);
	...
	private void writeToDataBase(PrintStream p, int rowNum, int colNum,
			Vector<String> vec, String outStr, String otherStr) {
