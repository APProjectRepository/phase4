...
public class ORMHandler {
	...
	public ORMHandler(String com) throws InvalidORClass,Exception {
		...
		if(command.toLowerCase().matches("orm.create(\\w+)")){
			setCraete(true);
			className = "os.math.sharif.edu.orm"+command.substring(command.indexOf("(")+1, command.indexOf(")"));
			...
			setCraete(false);
			className = "dos.math.sharif.edu.orm."+command.substring(command.indexOf("(")+1, command.indexOf(")"));
			orm = new ORM(Class.forName(className));
			selectQueryBuilder();
			Object obj = orm.createObject();
